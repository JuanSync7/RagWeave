...
export function normalizeGeneratedAnswer(markdown) {
    let text = markdown || "";
    if (!text) {
        return "";
    }
...
export function renderMarkdown(id, markdown) {
...
export function rerankSourceLink(metadata) {
...
export function formatChunkNumber(metadata) {
...
export function compactExcerpt(text, maxLen = 220) {
...
export async function renderRerankedOriginalDocs(results) {
...
export function commandSummary(commands) {
    if (!commands.length) {
...
export function setSuggestions(elementId, commands) {
    const out = byId(elementId);
    out.textContent = commands.map((cmd) => `/${cmd.name}`).join("  ");
}
export async function initSlashCommandHints() {
    try {

...
class GuardrailsRuntime:
    ...
    GUARDRAIL_BACKEND is not 'nemo' and the package is not installed (REQ-907).
    """
    ...
    def is_enabled(cls) -> bool:
        ...
        Returns:
            True if GUARDRAIL_BACKEND is 'nemo' and guardrails have not been
            auto-disabled due to a prior initialization failure.
        """
        from config.settings import GUARDRAIL_BACKEND

        return GUARDRAIL_BACKEND == "nemo" and not cls._auto_disabled
    ...
    def initialize(self, config_dir: str) -> None:
        ...
        with type(self)._lock:
            ...
            if not self.is_enabled():
                logger.info("NeMo Guardrails disabled (GUARDRAIL_BACKEND is not 'nemo')")
                return

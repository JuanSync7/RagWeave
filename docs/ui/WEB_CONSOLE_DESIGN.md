...
Version: 3.0 | Status: Active | Domain: Web Console (Dual-Console Architecture)

| Version | Date | Author | Changes |
|---------|------|--------|---------|
| 3.0 | 2026-03-23 | AI Assistant | Renamed from Implementation Guide to Design Document per doc-authoring skill chain (Spec → Design → Implementation). Updated document intent and cross-references. |
...
| 1.0 | 2026-03-13 | AI Assistant | Initial design (originally named Implementation Guide) for single-console architecture |

> **Document intent:** This is the **design document** for the web console, sitting between the
> requirements specification (`WEB_CONSOLE_SPEC.md`) and the implementation plan (`WEB_CONSOLE_IMPLEMENTATION.md`).
> It describes the task-oriented architecture, component decomposition, phasing, dependency graph,
> code patterns, and reference implementations that guide the actual build.
> The companion HTML mockup (`task-4-1-preview.html`) serves as the visual design reference for the User Console.
> For the requirements specification, see `WEB_CONSOLE_SPEC.md`.
...
5. Render attached context chips above the input with a remove (x) button on each; include chip metadata in the query payload as `context_attachments[]`
...
8. Implement keyboard navigation in the command picker (up/down to move, Enter to execute/insert, Esc to dismiss); also support mouse/touch selection
...
+-- Task 4.6: Context Window Indicator <-- Task 4.1, 4.2, 1.4 ------------------+
+-- Task 4.7: Input Bar Context Actions <-- Task 4.3, 2.1 ----------------------+
+-- Task 4.8: Sidebar Navigation Rail <-- Task 4.1, 4.4 ------------------------+
                                                                                   |
Phase 5 (Conversation Management)                                                  |
...
| 4.6 Context Window Indicator | REQ-214 |
| 4.7 Input Bar Context Actions | REQ-215, REQ-216 |
| 4.8 Sidebar Navigation Rail | REQ-202, REQ-217 |
| **Phase 5: Conversation Management** | |

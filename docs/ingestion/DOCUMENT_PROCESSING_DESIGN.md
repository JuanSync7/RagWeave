...
| Field | Value |
|-------|-------|
| **Document** | Document Processing Pipeline Design Document |
| **Version** | 1.0.0 |
| **Status** | Active |
| **Spec Reference** | `DOCUMENT_PROCESSING_SPEC.md` v1.0.0 (FR-101–FR-587) |
| **Companion Documents** | `DOCUMENT_PROCESSING_SPEC.md`, `DOCUMENT_PROCESSING_SPEC_SUMMARY.md`, `EMBEDDING_PIPELINE_DESIGN.md`, `INGESTION_PIPELINE_ENGINEERING_GUIDE.md`, `INGESTION_NEW_ENGINEER_ONBOARDING_CHECKLIST.md`, `INGESTION_PLATFORM_SPEC.md` |
| **Created** | 2026-03-20 |
...
>
> See `EMBEDDING_PIPELINE_DESIGN.md` for the downstream phase that reads from the Clean
> Document Store.
...
See `EMBEDDING_PIPELINE_DESIGN.md` → Task S.2 for the reader implementation.
...
> **Scope note:** FR-510 and FR-511 are partially covered here — this task generates and attaches provenance metadata (source URI, positional span mapping) to refactored output. Chunk-level provenance propagation and citation resolution are downstream responsibilities covered in `EMBEDDING_PIPELINE_DESIGN.md`.
...
def compute_clean_hash(clean_markdown: str) -> str:
    ...
    See EMBEDDING_PIPELINE_DESIGN.md → Task S.2 for the downstream
    usage of this hash.
    """
